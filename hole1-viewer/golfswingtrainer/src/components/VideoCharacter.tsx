...
interface VideoCharacterProps {
  videoPath: string;
  characterId: string;
  videoType?: 'driver' | 'idle' | 'iron';  // Video type to read config from
...
export function VideoCharacter({
  ...
  // Get video config from motion config (new structure: characters.{id}.video.{type})
  const videoConfig = useMemo(() => {
    return config?.characters?.[characterId]?.video?.[videoType] || {
      offset: { x: 0, y: 0, z: 0 },
      pivot: { x: 0, y: 0 },
      scale: 1,
      chromaKey: { color: '#00ff00', threshold: 0.4, smoothing: 0.1 },
    };
  }, [config, characterId, videoType]);
