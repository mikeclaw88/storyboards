...
import { CLUB_MAP } from '../config/clubs';
...
export function GolfBall({ ballConfig }: GolfBallProps) {
  ...
  // Get surface type for surface-aware physics
  const getSurfaceType = useTerrainStore((s) => s.getSurfaceTypeAtWorldPosition);
  ...
  useEffect(() => {
    if (ball.isFlying && swingResult && !flightStateRef.current) {
      const club = CLUB_MAP[selectedClub];
  ...
  useFrame((_, delta) => {
    ...
    if (ball.isFlying && flightStateRef.current && flightStateRef.current.phase !== 'stopped' && !hasLandedRef.current) {
      const newState = updateFlightPosition(flightStateRef.current, delta, getTerrainHeight, getSurfaceType);
