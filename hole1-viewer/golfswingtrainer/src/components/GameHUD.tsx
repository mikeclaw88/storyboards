...
export function GameHUD() {
  ...
  // Distance to hole from current ball position (idle display)
  const distToHole = Math.sqrt(
    Math.pow(ball.position[0] - holePosition[0], 2) +
    Math.pow(ball.position[2] - holePosition[2], 2)
  );
  const distToHoleYards = Math.round(metersToYards(distToHole));

  // Show distance to hole when idle (ready/pulling), live distance when in flight
  const isIdle = swingPhase === 'ready' || swingPhase === 'pulling';
      ...
      <div className="absolute top-6 left-6 z-50 font-mono text-white pointer-events-none">
        ...
        {showResult && (
          <div className="mt-4 bg-black/70 p-4 rounded-lg backdrop-blur-md border border-[#4ade80] animate-in fade-in slide-in-from-left-4 duration-300 w-64 pointer-events-auto relative z-50">
            {/* Header: STROKE # + shot type badge */}
            <div className="flex items-center justify-between mb-2 border-b border-gray-600 pb-2">
              <span className="text-lg font-bold text-white">STROKE {currentShot}</span>
