...
function colorizeHumanSprite(type, team) {
    ...
    for (let oy = 0; oy < OUTPUT_SIZE; oy++) {
        for (let ox = 0; ox < OUTPUT_SIZE; ox++) {
            ...
            // White on left half AND no mask data = background (transparent)
            if (Math.min(lr, lg, lb) > 240 && totalMask < MASK_THRESHOLD) {
                out[outI + 3] = 0;
                continue;
            }
            if (totalMask < MASK_THRESHOLD) {
                // No mask data — outline (dark) or pass through left-side color
                if (luminance < 0.3) {
                    // Outline - black
                    out[outI] = 0;
                    out[outI + 1] = 0;
                    ...
                    // No mask but not white background — use left-side color as-is
                    out[outI] = lr;
                    out[outI + 1] = lg;
                    out[outI + 2] = lb;
                    out[outI + 3] = 255;
