...
function colorizeHumanSprite(type, team) {
    ...
    // Helper: check if a source pixel (left half) is transparent
    const isTransparent = (sx, sy) => {
        if (sx < 0 || sy < 0 || sx >= halfW || sy >= imgHeight)
            return true;
        return src[(sy * imgWidth + sx) * 4 + 3] < 128;
    };
    for (let oy = 0; oy < OUTPUT_SIZE; oy++) {
        for (let ox = 0; ox < OUTPUT_SIZE; ox++) {
            ...
            if (totalMask < MASK_THRESHOLD) {
                ...
                else if (isTransparent(srcX - 1, srcY) || isTransparent(srcX + 1, srcY) ||
                    isTransparent(srcX, srcY - 1) || isTransparent(srcX, srcY + 1)) {
                    // Bright pixel at character edge (adjacent to transparent) = fringe, skip
                    out[outI + 3] = 0;
                }
                else {
                    // Interior bright pixel with no mask (eye whites) — pass through
                    out[outI] = lr;
                    out[outI + 1] = lg;
                    out[outI + 2] = lb;
                    out[outI + 3] = 255;
