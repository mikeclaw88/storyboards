...
class Unit extends GameObject {
    constructor(position, sizeFactor = 1, team, type) {
        ...
        this.getImage(type, (team == 2 /* Team.Bravo */));
        this.loadProperties();
    ...
    getImage(index = 0, mutate = false) {
        this.image = Unit.prepareImage(index, mutate);
    }
    static prepareImage(index = 0, mutate = false) {
        ...
        // ctx.drawImage(appSprite, 0 + (32 * Math.floor(index % 8)), 32 * Math.floor(index / 8), 32, 32, 0+1, 0+1, 32, 32); // 
        switch (index) {
            case 3 /* EntityType.Knight */:
                index = 1;
                break;
            case 5 /* EntityType.Cavalry */:
                index = 2;
                break;
            default: index = 0;
        }
...
class Artillery extends Archer {
    constructor(position, sizeFactor, team) {
        ...
        // force image
        this.getImage(4 /* EntityType.Artillery */, (team == 2 /* Team.Bravo */));
        this.loadProperties();
        this.bulletSpeed = 10;
    }
...
class GameState {
    ...
    init(level) {
        ...
        unitTypes.forEach(unitType => {
            if (!levelUnitTypes.includes(unitType)) {
                ...
                if (unitType == 4 /* EntityType.Artillery */)
                    this.btnArtillery.enabled = false;
                if (unitType == 5 /* EntityType.Cavalry */)
    ...
    drawUI() {
        ...
        // Score stars
        if (this.battleStatus == 2 /* BattleStatus.figth */ && this.teamAlpha.length > 0) {
    ...
    createGameButtons() {
        ...
        for (const unitType of unitTypes) {
            ...
            if (unitType == 4 /* EntityType.Artillery */)
                this.btnArtillery = button;
            if (unitType == 5 /* EntityType.Cavalry */)
                this.btnCavalry = button;
