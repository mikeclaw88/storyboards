...
// Pre-render the SVG sword to an offscreen canvas for reliable drawing.
// The SVG is blade-UP (blade y=1..10, crossguard y=10..12, handle y=12..15).
// We flip it vertically on the canvas so blade points DOWN (+Y), matching game coords.
const SWORD_W = 16;
const SWORD_H = 16;
const swordCanvas = document.createElement('canvas');
swordCanvas.width = SWORD_W;
swordCanvas.height = SWORD_H;
const swordSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${SWORD_W}" height="${SWORD_H}" viewBox="0 0 16 16" shape-rendering="crispEdges">
  <rect x="7" y="1" width="2" height="10" fill="#333"/>
  <rect x="6" y="2" width="4" height="8" fill="#333"/>
  <rect x="7" y="2" width="2" height="8" fill="#E6E6E6"/>
  <rect x="7" y="2" width="1" height="8" fill="#FFFFFF"/>
  <rect x="8" y="3" width="1" height="7" fill="#B3B3B3"/>
  <rect x="5" y="10" width="6" height="2" fill="#333"/>
  <rect x="5" y="10" width="6" height="1" fill="#FFD700"/>
  <rect x="5" y="11" width="6" height="1" fill="#C5A000"/>
  <rect x="7" y="12" width="2" height="3" fill="#333"/>
...
_swordImg.onload = () => {
    ...
    // Flip vertically so blade points down
    sCtx.translate(0, SWORD_H);
    sCtx.scale(1, -1);
    sCtx.drawImage(_swordImg, 0, 0, SWORD_W, SWORD_H);
};
_swordImg.src = 'data:image/svg+xml,' + encodeURIComponent(swordSvg);
class UnitSword extends BodyPart {
    ...
    _draw(ctx, offsets = { _position: new Vector(0, 0), r: 0 }) {
        ...
        ctx.translate(this.Position.x + offsets._position.x, this.Position.y + offsets._position.y);
        ctx.rotate(this._rotation + offsets.r);
        const h = this._length + 5;
        const w = this._size.x;
        ctx.imageSmoothingEnabled = false;
        // Draw from swordCanvas (already flipped blade-down).
        // Source: full 16x16 canvas. Dest: (0, 0) to (w, h) matching old fillRect bounds.
        ctx.drawImage(swordCanvas, 0, 0, SWORD_W, SWORD_H, 0, 0, w, h);
        ctx.restore();
    }
}
