...
function createUnitsDatabase() {
    ...
    allDataBase.push({
        ...
        cost: 120, health: 75, armor: 20, speedFactor: .2,
...
class Level {
    constructor() {
        ...
        let dataTable = [
            [12, 0 /* EntityType.Troop */, themeDef.forest, 2, 0],
            [50, 0 /* EntityType.Troop */, themeDef.dessert, 1.5, 0],
...
class GameState {
    ...
    battleEnd() {
        if (this.battleStatus == 2 /* BattleStatus.figth */) {
            ...
            if (result != 0) {
                ...
                setTimeout(() => {
                    if (result === 1 /* GameResult.win */ && this.level.levelIndex < this.level.finalLevelIndex) {
                        this.next();
                    }
                    else if (result === 1 /* GameResult.win */) {
    ...
    calculateUnitsAvailable() {
        const countByType = (type) => this.teamAlpha.filter(u => u.type === type).length;
        if (this.btnTroop)
            this.btnTroop.data = '' + countByType(0 /* EntityType.Troop */);
        if (this.btnTestudo)
            this.btnTestudo.data = '' + countByType(1 /* EntityType.Testudo */);
        if (this.btnArcher)
            this.btnArcher.data = '' + countByType(2 /* EntityType.Archer */);
        if (this.btnArtillery)
            this.btnArtillery.data = '' + countByType(4 /* EntityType.Artillery */);
        if (this.btnCavalry)
            this.btnCavalry.data = '' + countByType(5 /* EntityType.Cavalry */);
    }
    createGameButtons() {
        this.buttons = [];
        this.placeButtons = [];
