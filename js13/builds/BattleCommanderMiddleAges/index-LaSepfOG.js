...
const battlefieldUrl = ""+new URL('battlefield.jpg', import.meta.url).href+"";
...
function teamTint(color, team, strength) {
    const [r, g, b] = color;
    if (team === 1 /* Team.Alpha */) {
        return [
...
function colorizeHumanSprite(type, team) {
    ...
    // Helper: check if a neighbor has mask data on the right half
    const neighborHasMask = (sx, sy) => {
        if (sx < 0 || sy < 0 || sx >= halfW || sy >= imgHeight)
            return false;
        const ni = (sy * imgWidth + (sx + halfW)) * 4;
        const nr = src[ni] / 255;
        const ng = src[ni + 1] / 255;
        const nb = src[ni + 2] / 255;
        return (Math.min(nr, ng, nb) + Math.max(0, nr - Math.min(nr, ng, nb)) +
            Math.max(0, ng - Math.min(nr, ng, nb)) + Math.max(0, nb - Math.min(nr, ng, nb))) >= MASK_THRESHOLD;
    };
        ...
        for (let ox = 0; ox < outW; ox++) {
            ...
            if (totalMask < MASK_THRESHOLD) {
                if (luminance < 0.3) {
                    ...
                    // Bright pixel with no mask — only keep if surrounded by masked pixels
                    // (catches eye whites which are small holes in the mask)
                    const maskedNeighbors = (neighborHasMask(srcX - 1, srcY) ? 1 : 0) +
                        (neighborHasMask(srcX + 1, srcY) ? 1 : 0) +
                        (neighborHasMask(srcX, srcY - 1) ? 1 : 0) +
                        (neighborHasMask(srcX, srcY + 1) ? 1 : 0);
                    if (maskedNeighbors >= 2) {
                        // Interior hole in mask (eye whites) — pass through left-side color
                        out[outI] = lr;
                        out[outI + 1] = lg;
                        out[outI + 2] = lb;
                        out[outI + 3] = 255;
                        ...
                        // Unmapped boundary pixel — transparent
                        out[outI + 3] = 0;
                    }
                }
            }
...
const crossbowUrl = ""+new URL('humancrossbow.svg', import.meta.url).href+"";
...
class Archer extends Unit {
    ...
    drawChilds(ctx, realSize) {
        if (!crossbowImg.complete)
            return;
        ctx.translate(this.Size.x * -0.2, realSize.y * .05);
...
class UnitStick extends BodyPart {
    ...
    _draw(ctx, offsets = { _position: new Vector(0, 0), r: 0 }) {
        ...
        ctx.translate(this.Position.x + offsets._position.x + this._size.x, this.Position.y + offsets._position.y);
...
class GameState {
    ...
    onUpdate(dt) {
        ...
        for (const d of this.dmgNumbers) {
            ...
            ctx.fillStyle = d.team === 1 /* Team.Alpha */ ? '#f44' : '#48f';
