...
export function useMotionConfig(): UseMotionConfigResult {
   ...
   * For video characters: reads from config.video[characterId].offset
   */
  const getCharacterPosition = useCallback(
    (characterId: string): Vector3 => {
      // Check 3D character config first
      const charConfig = config.characters[characterId];
      if (charConfig?.position) {
        return charConfig.position;
      }
      // Check video character config (uses 'offset' instead of 'position')
      const videoConfig = config.video?.[characterId];
   ...
   * For video characters: reads from config.video[characterId].impactTime
  ...
  const getImpactTime = useCallback(
    (characterId: string, animationId: string): number | undefined => {
      // Check video config first (per-character timing)
      const videoConfig = config.video?.[characterId];
   ...
   * For video characters: reads from config.video[characterId].backswingTopTime
