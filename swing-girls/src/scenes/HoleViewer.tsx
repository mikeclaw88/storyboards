...
// Simple directional light diffuse calculation
...
// Note: This requires full screen pass usually, but we can fake it on terrain 
// by checking neighbor normals or depth
const edgeFragment = `
  float getEdge(vec2 uv) {
    // Basic texture edge detection
...
function TerrainRenderer({ mode, heightMap, detailMap, heightScale }: { mode: string, heightMap: THREE.Texture, detailMap: THREE.Texture, heightScale: number }) {
  ...
  const uniforms = useMemo(() => ({
    ...
    uTime: { value: 0 }
  }), [heightMap, detailMap]); // heightScale updated via ref

  // === VERTEX SHADER (Shared) ===
  // Calculates height displacement and passes data
  const baseVertex = `
    ...
    void main() {
      ...
      // Calculate normal from heightmap (finite difference)
      float off = 1.0 / 512.0;
      ...
      vec3 pos = position + vec3(0, 0, h * scale); // Z is up in PlaneGeometry
      
      vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
      vViewPosition = -mvPosition.xyz;
      gl_Position = projectionMatrix * mvPosition;
      
      gl_PointSize = (300.0 / length(mvPosition.xyz)); // For points mode
    }
  `;
        ...
        <shaderMaterial
          ...
          fragmentShader={`
            ...
            void main() {
              ...
              if (rim > 0.8) finalColor += vec3(0.2); // Highlight
              if (edge > 0.5) finalColor = vec3(0.0); // Black Outline
        ...
        <shaderMaterial
          ...
          fragmentShader={`
            ...
            void main() {
              ...
              // Halftone Pattern
              vec2 uv = gl_FragCoord.xy / 4.0; // Pattern size
              float dot = length(fract(uv) - 0.5) * 2.0; // 0..1 circle
              float radius = sqrt(1.0 - light); // Darker = larger dots
              
              vec3 finalColor = color;
              if (dot < radius) finalColor *= 0.5; // Darken dots
        ...
        <shaderMaterial
          ...
          fragmentShader={`
            ...
            void main() {
              ...
              // Mix original color with white paper
