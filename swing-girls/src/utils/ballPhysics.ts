...
// Ground physics
const BOUNCE_COEFFICIENT = 0.38; // Energy retained on bounce (golf ball on grass)
const ROLLING_FRICTION = 6.0; // Deceleration while rolling (rough grass - high friction)
...
export function calculateInitialVelocity(
  ...
  const speed = MIN_VELOCITY + curvedPower * (MAX_VELOCITY - MIN_VELOCITY);
...
export function updateFlightPosition(
  ...
  if (state.phase === 'rolling') {
    ...
    const frictionCanOvercomeSlope = ROLLING_FRICTION > slopeMagnitude;
    ...
    if (horizontalSpeed > 0) {
      const frictionDecel = ROLLING_FRICTION;
